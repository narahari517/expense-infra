...
resource "aws_lb_listener" "http" {
  ...
  default_action {
    ...
    fixed_response {
      ...
      message_body = "Hello, I am from ingress ALB"
      status_code  = "200"
    }
  }
}
...
module "records" {
  ...
  records = [
    {
      name    = "expense-${var.environment}" # *.app-dev.nhari.online
      type    = "A"
      alias   = {
...
resource "aws_lb_listener_rule" "expense" {
  ...
  condition {
    host_header {
      values = ["${var.project_name}-${var.environment}.${var.zone_name}"] # expense-dev.nhari.online
    }
  }
}